...
func TestMigrationVersionWALPermissionsAndUpgrade(t *testing.T) {
	...
	if err := upgraded.db.QueryRow(`SELECT MAX(version) FROM schema_migrations`).Scan(&version); err != nil || version != 10 {
...
func TestMigrationSevenRequeuesV071DeregisterDeadLetterExactlyOnce(t *testing.T) {
	...
	assertDrainMigrationRequeuedOnce(t, store, "v071-drain", 5, 10)
}
...
func TestMigrationEightRequeuesExhaustedOwnedDrainAfterReplacementJob(t *testing.T) {
	...
	assertDrainMigrationRequeuedOnce(t, store, "replacement-job-drain", 12, 10)
}
