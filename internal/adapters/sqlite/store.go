...
func (s *Store) Migrate(ctx context.Context) error {
	...
	if version < 9 {
		...
		if _, err := s.txExec(ctx, tx, "migrate.v9.drain", `INSERT INTO operations(
			...
			lifecycle.OperationDrain, operations.OperationPending, now, now, now,
			operations.OperationDead, lifecycle.OperationProvision, legacyStageAcquireJITError, operations.StateReachable); err != nil {
			return fmt.Errorf("migration 9 enqueue cleanup: %w", err)
		}
		if _, err := s.txExec(ctx, tx, "migrate.v9.dependency", `INSERT INTO operation_dependencies(operation_id,depends_on)
			SELECT 'recovery-drain-'||i.id,o.id FROM operations o JOIN instances i ON i.id=o.resource_id WHERE `+match,
			operations.OperationDead, lifecycle.OperationProvision, legacyStageAcquireJITError, operations.StateReachable); err != nil {
			return fmt.Errorf("migration 9 order cleanup: %w", err)
		}
		if _, err := s.txExec(ctx, tx, "migrate.v9.history", `INSERT INTO transition_history(instance_id,from_state,to_state,version,operation_id,created_at)
			SELECT i.id,i.state,?,i.version+1,'recovery-drain-'||i.id,? FROM operations o JOIN instances i ON i.id=o.resource_id WHERE `+match,
			operations.StateDraining, now, operations.OperationDead, lifecycle.OperationProvision,
			legacyStageAcquireJITError, operations.StateReachable); err != nil {
			return fmt.Errorf("migration 9 record cleanup transition: %w", err)
		}
		if _, err := s.txExec(ctx, tx, "migrate.v9.instance", `UPDATE instances SET state=?,version=version+1,drain_phase=1,updated_at=?
			...
			AND EXISTS (SELECT 1 FROM operations o WHERE o.id=json_extract(instances.ownership,'$.operation_id')
				...
				AND o.effect_key=o.kind||':'||o.resource_id
				AND NOT EXISTS (SELECT 1 FROM operation_effects e WHERE e.operation_id=o.id))`,
			operations.StateDraining, now, operations.StateReachable, operations.OperationDead,
			lifecycle.OperationProvision, legacyStageAcquireJITError); err != nil {
			return fmt.Errorf("migration 9 transition orphan: %w", err)
		}
		if _, err := s.txExec(ctx, tx, "migrate.v9.provision", `UPDATE operations
			SET status=?,attempts=0,available_at=?,lease_owner='',lease_until=0,last_error='',updated_at=?
			...
			operations.OperationPending, now, now, operations.OperationDead, lifecycle.OperationProvision,
			legacyStageAcquireJITError); err != nil {
			return fmt.Errorf("migration 9 revive provision: %w", err)
		}
		if _, err := s.txExec(ctx, tx, "migrate.v9.record", `INSERT INTO schema_migrations(version, applied_at) VALUES(9, ?)`, now); err != nil {
