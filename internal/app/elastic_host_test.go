...
func TestHostObservationReportsPhysicalCapacityWhenElastic(t *testing.T) {
	observation := hostObservation(productionSnapshot(), configuredCapacity(), productionGuards(), true, domain.Resources{}, false)
...
func TestHostObservationYieldsCPUAsHostGetsBusy(t *testing.T) {
	...
	for _, idle := range []float64{2, 15, 40, 80, 100} {
		...
		available := hostObservation(snapshot, configuredCapacity(), productionGuards(), true, domain.Resources{}, false).Value.Available.CPU
	...
	if got := hostObservation(saturated, configuredCapacity(), productionGuards(), true, domain.Resources{}, false).Value.Available.CPU; got != 0 {
...
func TestHostObservationClampsNonsenseIdle(t *testing.T) {
	for _, test := range []struct {
		...
		got := hostObservation(snapshot, configuredCapacity(), productionGuards(), true, domain.Resources{}, false).Value.Available.CPU
		if got != test.want {
...
func TestHostObservationLegacyModeUnchanged(t *testing.T) {
	host := hostObservation(productionSnapshot(), configuredCapacity(), productionGuards(), false, domain.Resources{}, false).Value
...
func TestHostObservationUnobservedPhysicalFactsFallBack(t *testing.T) {
	...
	host := hostObservation(snapshot, configuredCapacity(), productionGuards(), true, domain.Resources{}, false).Value
...
func TestHostObservationBlockedPressureStillFailsClosed(t *testing.T) {
	...
	observation := hostObservation(snapshot, configuredCapacity(), productionGuards(), true, domain.Resources{}, false)
