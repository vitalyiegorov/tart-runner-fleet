...
- **Scheduling is fair by construction.** Young work is ordered by dominant
  ...
  band, between demands of one profile and within one platform, a GitHub scope
  that already holds slots on this node yields the next one to a scope that
  holds none, so one scope's batch cannot own a two-slot Mac while another scope
  waits
